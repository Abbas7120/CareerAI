...
async function generateResumeContent(data, templateId) {
  const {
    fullName, email, phone, linkedin, portfolio,
    education, skills, summary, experience, achievements, extracurricular,
  } = data;
...
exports.generateResume = async (req, res) => {
  try {
    ...
    const resume = await generateResumeContent({
      fullName, email, phone, linkedin, portfolio, education, skills,
      summary:         enhancedSummary,
      experience:      enhancedExperience,
      achievements:    enhancedAchievements,
      extracurricular: enhancedExtracurricular,
    ...
    // ── Save to resumes table (existing) ──────────────────────────────────────
    try {
      await db.query(
        `INSERT INTO resumes (full_name, email, education, skills, experience, projects, generated_resume)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
      ...
      console.warn("[resume] DB save failed (non-critical):", dbErr.message);
    }

    // ── Save to user_outputs for History page ─────────────────────────────────
    if (clerkUserId) {
      try {
        await db.query(
          ...
          [
            ...
            email || "",
            "resume",
            `Resume for: ${fullName} — ${templateId} template`,
            resume,
          ]
        );
      } catch (dbErr) {
        console.warn("[resume] History save failed (non-critical):", dbErr.message);
      }
    }
